...
public class TextToSpeechController : MonoBehaviour
{
    ...
    //public string inputTextTest = "Once upon a time, there lived a girl called Alice. She lived in a house in the woods.";
    ...
    public void TextToSpeech(string inputText, out float audioSize)
    {
        string cleanedMessage = Regex.Replace(inputText, @"[^\p{L}\p{N} \r\n]", "");
        string ptext;
        if (hasPhenomeDictionary)
        {
            ptext = TextToPhonemes(cleanedMessage);
            //Debug.Log(ptext);
        }
        else
        {
        ...
        DoInference(ptext, out audioSize);
    }
    ...
    public void DoInference(string ptext, out float audioSize)
    {
        ...
        audioSize = samples.Length / samplerate;
